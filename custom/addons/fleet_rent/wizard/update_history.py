...
class UpdateTireInfo(models.TransientModel):
    ...
    previous_tire_size = fields.Char(string='Previous Tyre Size', size=124)
    new_tire_size = fields.Char(string="New Tyre Size", size=124)
    previous_tire_sn = fields.Char(string='Previous Tyre S/N', size=124)
    new_tire_sn = fields.Char(string="New Tyre S/N", size=124)
    previous_tire_issue_date = fields.Date(
        string='Previous Tyre Issuance Date')
    new_tire_issue_date = fields.Date(string='New Tyre Issuance Date')
    changed_date = fields.Date(string='Change Date',
                               default=datetime.now().date())
    note = fields.Text('Notes', translate=True)
    temp_bool = fields.Boolean(default=True, string='Temp Bool for making \
                                                previous Tyre info readonly')
    workorder_id = fields.Many2one('fleet.vehicle.log.services',
                                   string='Work Order')
    vehicle_id = fields.Many2one('fleet.vehicle', string="Vehicle")
        ...
        for vehicle in self:
            if vehicle.previous_tire_issue_date and \
                ...
                if vehicle.new_tire_issue_date < \
                    ...
                    raise ValidationError('New Tyre Issuance Date Should Be \
                    Greater Than Previous Tyre Issuance Date.')
        ...
        for vehicle in self:
            if vehicle.changed_date and vehicle.new_tire_issue_date:
                if vehicle.changed_date < vehicle.new_tire_issue_date:
                    raise ValidationError('Tyre Change Date Should Be \
                    Greater Than New Tyre Issuance Date.')
