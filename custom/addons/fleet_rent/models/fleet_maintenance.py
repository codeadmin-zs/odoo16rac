...
class FleetVehicleLogServices(models.Model):
    ...
    def unlink(self):
        for rec in self:
            if rec.state != 'draft':
                raise Warning(_('You can\'t delete Work Order which in Confirmed or Done state!'))
        return super(FleetVehicleLogServices, self).unlink()

    @api.onchange('vehicle_id')
    def onchange_vehicle_id(self):
        if self.vehicle_id:
            # if self.vehicle_id.vehicle_prodcut_template_id:
            self.expence_acc_id = self.vehicle_id.vehicle_prodcut_template_id.property_account_expense_id.id

    @api.onchange('vehicle_id')
    def get_vehicle_info(self):
        if self.vehicle_id:
            vehicle = self.vehicle_id
            # Values not updating in table, using update() to update the values
            #             self.vechical_type_id = vehicle.model_id.category_vehicle_class_id.id
            ...
            values = {
                # 'vechical_type_id': vehicle.vechical_type_id and vehicle.vechical_type_id.id or False,
                'vechical_type_id': vehicle.model_id.category_vehicle_class_id.id,
                'purchaser_id': vehicle.driver_id and vehicle.driver_id.id or False,
                'fmp_id': vehicle.name or "",
                'f_brand_id': vehicle.f_brand_id and vehicle.f_brand_id.id or False,
                # 'vehical_division_id': vehicle.vehical_division_id and vehicle.vehical_division_id.id or False,
                'vechical_location_id': vehicle.vechical_location_id and vehicle.vechical_location_id.id or False,
            }
            self.update(values)
    ...
    def action_confirm(self):
        ...
        for work_order in self:
            if work_order.vehicle_id.state_id.name == 'Available' or work_order.vehicle_id.state_id.name == 'In shop':
                # if work_order.vehicle_id.state_id.name != 'Available' or work_order.vehicle_id.state_id.name != 'In shop':
                # raise Warning(_("Vehicle Should be in Available or In shop State..!"))
                # if work_order.vehicle_id.state == 'write-off':
                #     raise Warning(_("You can\'t confirm this work order which vehicle is in write-off state!"))
                # if work_order.vehicle_id.state_id.name == 'In Workshop':
                #     raise Warning(_("Previous work order is not completed, "
                #                     "complete that work order first then you can confirm this work order!"))
                # elif work_order.vehicle_id.state == 'draft' or \
                #         work_order.vehicle_id.state == 'complete':
                #     raise Warning(_("Confirm work order can only when vehicle status is in Available or Released!"))
                state_id_temp = self.env['fleet.vehicle.state'].search([
                    ('name', '=', 'In Workshop')])
                work_order.vehicle_id.write({
                    'state': 'in_progress',
                    'state_id': state_id_temp.id,
    ...
    def add_service_days(self):
        context = {'active_model': 'fleet.vehicle',
                   'active_id': self.vehicle_id.id,
                   'default_id': self.vehicle_id.id}
        wiz_form_id = self.env.ref('fleet_rent.service_configuration_date_adding').id
        return {
    ...
    def action_done(self):
        ...
        if not self.repair_line_ids:
            raise ValidationError("Please Add the Maintenance Details and It's Status Before Closing..")

        else:
            # service_repair_line_obj = self.env['service.repair.line'].search([('service_id', '=', self.id)])
            # c_total = 0
            # for service_repair_line_obj_id in service_repair_line_obj:
            #     c_total += service_repair_line_obj_id.cost
            # self.update({'amount': c_total})
            # raise ValidationError('debug_error')
            for work_order in self:
                for repair_line in work_order.repair_line_ids:
                    # if repair_line.id is False:
                    #     raise ValidationError("Please Add the Maintenance Details and It's Status Before Closing..")
                    if repair_line.complete is True:
                        continue
                    elif repair_line.complete is False:
                        raise ValidationError("Work Order Should be Completed Before Closing...")
                        model_data_ids = mod_obj.search([
                            ('model', '=', 'ir.ui.view'),
            ...
            # next_service_day_ids = next_service_day_obj.sudo().search([
            #     ('vehicle_id', '=', work_order.vehicle_id.id)])
            # if not next_service_day_ids:
            #     form = "fleet_rent.service_configuration_warning_form"
            #     return {
            #         'name': 'Warning',
            ...
            for work_order in self:
                ...
                status_id_available = self.env['fleet.vehicle.state'].search([('name', '=', 'Available')]).id
                if work_order.vehicle_id:
                    work_order.vehicle_id.update({
                        'state': 'inspection',
                        'state_id': status_id_available,
                        'work_order_close': True})
                # if work_order.already_closed:
                #     for repair_line in work_order.repair_line_ids:
                #         for pending_repair_line in \
                #                 work_order.vehicle_id.pending_repair_type_ids:
                #             if repair_line.repair_type_id.id == pending_repair_line.repair_type_id.id and \
                #                     work_order.name == pending_repair_line.name:
                #                 if repair_line.complete is True:
                #                     pending_repair_line.unlink()
        sub_total = 0
        for service_repair_line in self.env['service.repair.line'].search([('service_id', '=', self.id)]):
            sub_total += service_repair_line.sub_total
        self.update({'amount': sub_total})

        return True

    # @api.multi
    def close_reopened_wo(self):
        """Method is used to update the existing shipment moves if WO parts \
        are updated in terms of quantities or complete parts."""
                ...
                if product.old_part_return:
                    if not scrap_pick_id:
                        ...
                        if out_pick_type and \
                            ...
                            src_loc = False
                            customerloc, src_loc = stock_wh._get_partner_locations()
                            if src_loc:
                                src_loc = src_loc.id

                        if out_pick_type and out_pick_type[0].default_location_dest_id:
                            dest_loc = out_pick_type[0].default_location_dest_id.id
                        else:
                            dest_loc = False
                            dest_loc, supplierloc = stock_wh._get_partner_locations()
                            if dest_loc:
                                dest_loc = dest_loc.id
                        delivery_dict.update({
                            'origin': work_order.name or '',
                    ...
                    if not scrap_dest_loc_ids:
                        raise Warning(_("There is no location defined for scrapped parts "
                                        "please define location with Scrapped!"))
                    if work_order.team_id:
                        scrap_source_loc_id = work_order.team_id and \
                                              work_order.team_id.id or False
                    else:
                        raise Warning(_("There is no team selected as source location, as old parts are "
                                        "returned so you need to select any for getting return parts.!"))
                    return_move_vals = {
                        'product_id': product.product_id and
                                      product.product_id.id or False,
                        'name': product.product_id and
        ...
        for order in self:
            if order.vehicle_id:
                if order.vehicle_id.state == 'write-off':
                    raise Warning(_("You can\'t Re-open this work order which vehicle is in write-off state!"))
                elif order.vehicle_id.state == 'in_progress':
                    raise Warning(_("Previous work order is not complete, "
        ...
        if self._context.get('active_ids', False):
            for vehicle in vehicle_obj.browse(self._context['active_ids']):
                if vehicle.state == 'write-off':
                    raise Warning(_("You can\'t create work order for vehicle which is already write-off!"))
                elif vehicle.state == 'in_progress':
                    raise Warning(_("Previous work order is not complete, "
                                    "complete that work order first than you can create new work order!"))
                elif vehicle.state == 'rent' or vehicle.state == 'booked':
                    raise Warning(_("You can\'t create work order for vehicle which is already On Rent or Booked!"))
                elif vehicle.state == 'draft' or vehicle.state == 'waiting':
                    raise Warning(_("You can\'t create work order for a vehicle which has not finished registration!"))
        res = super(FleetVehicleLogServices, self).default_get(fields)
        repair_type_ids = repair_type_obj.search([])
        if not repair_type_ids:
            raise Warning(_("There is no data for repair type, add repair type from configuration!"))
        return res
        ...
        for vehicle in self:
            if vehicle.date and vehicle.date_complete:
                if vehicle.date_complete < vehicle.date:
                    raise ValidationError('ETIC Date Should Be Greater Than Issue Date.')
    ...
    name = fields.Char(string='Work Order', size=32, readonly=True,
                       translate=True)
    fmp_id = fields.Char(string="Vehicle ID", size=64, readonly=True)
    ...
    date_open = fields.Date(string='Open Date',
                            help="When Work Order will confirm this date will be set.")
    ...
    repair_line_ids = fields.One2many(''
                                      'service.repair.line', 'service_id',
                                      string='Repair Lines')
    ...
    vechical_type_id = fields.Many2one('vehicle.category', string='Vehicle Type')
    ...
    # @api.multi
    def return_action_to_generate_cost_bill(self):
        """
        Create invoice for Rent Schedule.
        """
        # if self.invc_id:
        #     raise ValidationError('Bill Already Generated..')
        # else:
        journal_ids = self.env['account.journal'].search(
            [('type', '=', 'purchase')])
        if not self.expence_acc_id.id:
            raise Warning(_('Please Configure Expense Account.'))
        if not self.team_id.property_account_payable_id.id:
            raise Warning(_('Please Configure Payable Account.'))
        inv_line_main = {
            'origin': self.fmp_id or self.vehicle_id.name or "",
            'name': 'Service for ' + self.vehicle_id.name or "",
            'price_unit': self.amount or 0.00,
            'quantity': 1,
            'account_id': self.expence_acc_id.id or False,
            'account_analytic_id': self.vehicle_id.analytic_account_id.id or False,
        }
        ...
        inv_values = {
            'partner_id': self.team_id.id or False,
            ...
            # 'account_id': self.team_id.property_account_payable_id.id
            #               or False
        }
        ...
        repair_line_obj = self.env['service.repair.line'].search([('service_id', '=', self.id)])
        inv_line_ids = []
        for repair_line in repair_line_obj:
            inv_line_values = {
                # 'origin': self.fmp_id,
                'name': 'Service for ' + self.vehicle_id.name + " " + repair_line.repair_type_id.name or "",
                'price_unit': repair_line.cost or 0.00,
                'description': repair_line.repair_type_id.name or " ",
                'asset_category_id': self.vehicle_id.vehicle_prodcut_id.product_tmpl_id.asset_category_id.id or False,
    ...
    # @api.multi
    def return_action_to_view_cost_bill(self):
        """
    ...
    def _get_odometer(self):
        ...
        for record in self:
            ids = record.vehicle_id.id
            # GETTING ERROR WHEN UNCOMMENT THE ORM, TRIED TO USE THE SQL COMMAND
            vehicle_odometer = FleetVehicalOdometer.sudo().search(
                [('vehicle_id', '=', record.vehicle_id.id)], limit=1, order='value desc')
            # sql1 = "SELECT value FROM fleet_vehicle_odometer WHERE vehicle_id = %s ORDER BY value DESC LIMIT 1"
            # cr.execute(sql1, (ids,))
            # checking = cr.dictfetchall()
            # if len(checking) > 0:
            #     record.odometer = checking[0]['value']
            # else:
            #     record.odometer = 0
            if vehicle_odometer:
...
class FleetWorkOrderSearch(models.TransientModel):
        ...
        for vehicle in self:
            if vehicle.issue_date_to:
                if vehicle.issue_date_to < vehicle.issue_date_from:
                    raise ValidationError('Issue To Date Should Be Greater Than Last Issue From Date.')
        ...
        for vehicle in self:
            if vehicle.open_date_to:
                if vehicle.open_date_to < vehicle.open_date_from:
                    raise ValidationError('Open To Date Should Be Greater Than Open From Date.')
        ...
        for vehicle in self:
            if vehicle.close_date_to:
                if vehicle.close_date_to < vehicle.close_date_form:
                    raise ValidationError('Close To Date Should Be Greater Than Close From Date.')
    ...
    def get_work_order_detail_by_advance_search(self):
        ...
        for order in self:
            ...
            repair_type_id = order.repair_type_id and order.repair_type_id.id or False
            if repair_type_id:
                ...
                if wrk_ids:
                    for wk_order in wrk_ids:
                        if wk_order.date_open:
                            diff = (datetime.today() -
                                    datetime.strptime(str(wk_order.date_open), DEFAULT_SERVER_DATE_FORMAT)).days
                            if str(diff) == wk_order.open_days:
                                order_ids.append(wk_order.id)
                    order_ids = sorted(set(order_ids))

            if fmp_id:
                work_order_ids = work_order_obj.search([('vehicle_id', '=', fmp_id)])
                if work_order_ids:
                    for work_line in work_order_ids:
                        order_ids.append(work_line.id)
                    order_ids = sorted(set(order_ids))

            # division_id = order.vehical_division_id and order.vehical_division_id.id or False
            # if division_id:
            #     vehicle_ids = vehicle_obj.search([('vehical_division_id', '=', division_id)])
            #     work_order_ids = work_order_obj.search([('vehicle_id', 'in', vehicle_ids.ids)])
            #     if work_order_ids:
            #         for work_line in work_order_ids:
            #             order_ids.append(work_line.id)
            #         order_ids = sorted(set(order_ids))
...
class FleetTeam(models.Model):
    ...
    def get_return_from_trip(self):
        ...
        for team in self:
            if team.state != 'sent':
                raise Warning(_("Please send your trip before getting return it!"))
            if not team.allocate_part_ids:
                raise Warning(_("parts are not loaded!"))
            if not team.outgoing_ship_ids:
                raise Warning(_("parts are not Delivered yet you can\'t get return it!"))
            if team.incoming_ship_ids:
                team_in_pick_id = team.incoming_ship_ids[0].id
                if team.incoming_ship_ids[0].move_lines:
                    team.incoming_ship_ids[0].move_lines.write({'state': 'draft'})
    ...
    def close_trip(self):
        ...
        for team in self:
            if not team.is_return:
                raise Warning(_("Please Fill the Used Part Info and than close the team Trip!"))
            if team.allocate_part_ids:
            ...
            if team.incoming_rem_ship_ids:
                ...
                if team.incoming_rem_ship_ids[0].move_lines:
                    team.incoming_rem_ship_ids[0].move_lines.write({'state': 'draft'})
...
class ServiceRepairLine(models.Model):
        ...
        for vehicle in self:
            if vehicle.target_date and vehicle.date_complete:
                if vehicle.target_date > vehicle.date_complete:
                    raise ValidationError('Target Date Should Be Less Than ETIC Date.')


    service_id = fields.Many2one('fleet.vehicle.log.services',
                                 ondelete='cascade')
    repair_type_id = fields.Many2one('product.template', string='Repair Type', domain="[('service_ok', '=', True)]",)
    ...
    cost = fields.Float(string='Amount')
    ...
    @api.depends('cost', 'quantity', 'tax_ids')
    def _compute_subtotal(self):
        for record in self:
            subtotal = record.cost * record.quantity

            for tax in record.tax_ids:
                subtotal += subtotal * (tax.amount / 100)

            record.sub_total = subtotal

    @api.onchange('repair_type_id')
    def _onchange_repair_type_cost_id(self):
        if self.repair_type_id:
            self.cost = self.repair_type_id.standard_price
        else:
            self.cost = 0.0

    @api.onchange('repair_type_id')
    def _onchange_repair_type_tax_id(self):
        if self.repair_type_id:
            self.tax_ids = self.repair_type_id.taxes_id
        else:
            self.tax_ids = None

    @api.onchange('repair_type_id')
    def _onchange_repair_type_description_id(self):
        if self.repair_type_id:
            self.description = self.repair_type_id.description
        else:
            self.description = ""
...
class VehicleMaintenanceName(models.Model):
    _rec_name = 'vehicle_id'
    _inherit = 'fleet.vehicle.log.services'

    # Override the service_type_id field
    service_type_id = fields.Many2one(
        'fleet.service.type',
        string='Service Type',
        default=lambda self: self._get_default_service_type(),
        store=True,
    )

    @api.model
    def _get_default_service_type(self):
        first_service_type = self.env['fleet.service.type'].search([('id', '=', 8)])
        return first_service_type or False
